using System;
using System.Runtime.CompilerServices;
using System.Runtime.InteropServices;

namespace AsphodelusNative.DirectWrite;

/// <summary>
/// INTERFACE <b>IDWriteBitmapRenderTarget</b> : IUnknown<br/>
/// DWRITE.h
/// </summary>
unsafe public interface IBitmapRenderTarget : IUnknown {
	/// <summary>
	/// STDMETHOD <b>HRESULT IDWriteBitmapRenderTarget::DrawGlyphRun (FLOAT baselineOriginX, FLOAT baselineOriginY, DWRITE_MEASURING_MODE measuringMode, DWRITE_GLYPH_RUN* glyphRun, IDWriteRenderingParams* renderingParams, COLORREF textColor, RECT* NULL)</b><br/>
	/// </summary>
	/// <param name="baselineOriginX"><b>FLOAT baselineOriginX</b></param>
	/// <param name="baselineOriginY"><b>FLOAT baselineOriginY</b></param>
	/// <param name="measuringMode"><b>DWRITE_MEASURING_MODE measuringMode</b></param>
	/// <param name="glyphRun">_In_ <b>DWRITE_GLYPH_RUN* glyphRun</b></param>
	/// <param name="renderingParams">_In_ <b>IDWriteRenderingParams* renderingParams</b></param>
	/// <param name="textColor"><b>COLORREF textColor</b></param>
	/// <param name="nULL">_Out_opt_ <b>RECT* NULL</b></param>
	int DrawGlyphRun<T0> (float baselineOriginX, float baselineOriginY, MeasuringMode measuringMode, GlyphRun* glyphRun, T0* renderingParams, uint textColor, Vector4I* nULL) where T0 : unmanaged, IRenderingParams;
	/// <summary>
	/// STDMETHOD <b>HRESULT IDWriteBitmapRenderTarget::DrawGlyphRun (FLOAT baselineOriginX, FLOAT baselineOriginY, DWRITE_MEASURING_MODE measuringMode, DWRITE_GLYPH_RUN* glyphRun, IDWriteRenderingParams* renderingParams, COLORREF textColor, RECT* NULL)</b><br/>
	/// </summary>
	/// <param name="baselineOriginX"><b>FLOAT baselineOriginX</b></param>
	/// <param name="baselineOriginY"><b>FLOAT baselineOriginY</b></param>
	/// <param name="measuringMode"><b>DWRITE_MEASURING_MODE measuringMode</b></param>
	/// <param name="glyphRun">_In_ <b>DWRITE_GLYPH_RUN* glyphRun</b></param>
	/// <param name="renderingParams">_In_ <b>IDWriteRenderingParams* renderingParams</b></param>
	/// <param name="textColor"><b>COLORREF textColor</b></param>
	/// <param name="nULL">_Out_opt_ <b>RECT* NULL</b></param>
	int DrawGlyphRun<T0> (float baselineOriginX, float baselineOriginY, MeasuringMode measuringMode, GlyphRun* glyphRun, T0* renderingParams, uint textColor, out Vector4I nULL) where T0 : unmanaged, IRenderingParams;
	/// <summary>
	/// STDMETHOD_ <b>HDC IDWriteBitmapRenderTarget::GetMemoryDC ()</b><br/>
	/// </summary>
	IntPtr GetMemoryDC ();
	/// <summary>
	/// STDMETHOD_ <b>FLOAT IDWriteBitmapRenderTarget::GetPixelsPerDip ()</b><br/>
	/// </summary>
	float GetPixelsPerDip ();
	/// <summary>
	/// STDMETHOD <b>HRESULT IDWriteBitmapRenderTarget::SetPixelsPerDip (FLOAT pixelsPerDip)</b><br/>
	/// </summary>
	/// <param name="pixelsPerDip"><b>FLOAT pixelsPerDip</b></param>
	int SetPixelsPerDip (float pixelsPerDip);
	/// <summary>
	/// STDMETHOD <b>HRESULT IDWriteBitmapRenderTarget::GetCurrentTransform (DWRITE_Matrix3x2F* transform)</b><br/>
	/// </summary>
	/// <param name="transform">_Out_ <b>DWRITE_Matrix3x2F* transform</b></param>
	int GetCurrentTransform (Matrix3x2F* transform);
	/// <summary>
	/// STDMETHOD <b>HRESULT IDWriteBitmapRenderTarget::GetCurrentTransform (DWRITE_Matrix3x2F* transform)</b><br/>
	/// </summary>
	/// <param name="transform">_Out_ <b>DWRITE_Matrix3x2F* transform</b></param>
	int GetCurrentTransform (out Matrix3x2F transform);
	/// <summary>
	/// STDMETHOD <b>HRESULT IDWriteBitmapRenderTarget::SetCurrentTransform (DWRITE_Matrix3x2F* transform)</b><br/>
	/// </summary>
	/// <param name="transform">_In_opt_ <b>DWRITE_Matrix3x2F* transform</b></param>
	int SetCurrentTransform (Matrix3x2F* transform);
	/// <summary>
	/// STDMETHOD <b>HRESULT IDWriteBitmapRenderTarget::GetSize (SIZE* size)</b><br/>
	/// </summary>
	/// <param name="size">_Out_ <b>SIZE* size</b></param>
	int GetSize (Vector2I* size);
	/// <summary>
	/// STDMETHOD <b>HRESULT IDWriteBitmapRenderTarget::GetSize (SIZE* size)</b><br/>
	/// </summary>
	/// <param name="size">_Out_ <b>SIZE* size</b></param>
	int GetSize (out Vector2I size);
	/// <summary>
	/// STDMETHOD <b>HRESULT IDWriteBitmapRenderTarget::Resize (UINT32 width, UINT32 height)</b><br/>
	/// </summary>
	/// <param name="width"><b>UINT32 width</b></param>
	/// <param name="height"><b>UINT32 height</b></param>
	int Resize (uint width, uint height);
}

/// <summary>
/// Instance of <b>IDWriteBitmapRenderTarget</b><br/>
/// DWRITE.h
/// </summary>
unsafe public readonly struct BitmapRenderTarget : IBitmapRenderTarget {
//unsafe public readonly struct IBitmapRenderTargetObj : IBitmapRenderTarget {

	readonly void** _pointer;

	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	readonly BitmapRenderTarget* GetCurrentPointer () => (BitmapRenderTarget*) Unsafe.AsPointer (ref Unsafe.AsRef (in this));
	//readonly IBitmapRenderTargetObj* GetCurrentPointer () => (IBitmapRenderTargetObj*) Unsafe.AsPointer (ref Unsafe.AsRef (in this));

	/// <inheritdoc />
	public static Guid GUID { get => _guid; }
	static readonly Guid _guid = new (0x5e5a32a3, 0x8dff, 0x4773, 0x9f, 0xf6, 0x06, 0x96, 0xea, 0xb7, 0x72, 0x67);

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public readonly int QueryInterface (Guid riid, void** ppvObject) {
		var ptr = GetCurrentPointer ();
		var hr = ((FunctionPointer*) ptr->_pointer)->QueryInterface (ptr, riid, ppvObject);
		return hr;
	}

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public readonly int QueryInterface (Guid riid, out void* ppvObject) {
		fixed (void** _ppvObject = &ppvObject) {
			var ptr = GetCurrentPointer ();
			var hr = ((FunctionPointer*) ptr->_pointer)->QueryInterface (ptr, riid, _ppvObject);
			return hr;
		}
	}

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public readonly uint AddRef () {
		var ptr = GetCurrentPointer ();
		var hr = ((FunctionPointer*) ptr->_pointer)->AddRef (ptr);
		return hr;
	}

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public readonly uint Release () {
		var ptr = GetCurrentPointer ();
		var hr = ((FunctionPointer*) ptr->_pointer)->Release (ptr);
		return hr;
	}

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public readonly int DrawGlyphRun<T0> (float baselineOriginX, float baselineOriginY, MeasuringMode measuringMode, GlyphRun* glyphRun, T0* renderingParams, uint textColor, Vector4I* nULL) where T0 : unmanaged, IRenderingParams {
		var ptr = GetCurrentPointer ();
		var hr = ((FunctionPointer*) ptr->_pointer)->DrawGlyphRun (ptr, baselineOriginX, baselineOriginY, measuringMode, glyphRun, renderingParams, textColor, nULL);
		return hr;
	}

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public readonly int DrawGlyphRun<T0> (float baselineOriginX, float baselineOriginY, MeasuringMode measuringMode, GlyphRun* glyphRun, T0* renderingParams, uint textColor, out Vector4I nULL) where T0 : unmanaged, IRenderingParams {
		fixed (Vector4I* _nULL = &nULL) {
			var ptr = GetCurrentPointer ();
			var hr = ((FunctionPointer*) ptr->_pointer)->DrawGlyphRun (ptr, baselineOriginX, baselineOriginY, measuringMode, glyphRun, renderingParams, textColor, _nULL);
			return hr;
		}
	}

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public readonly IntPtr GetMemoryDC () {
		var ptr = GetCurrentPointer ();
		var hr = ((FunctionPointer*) ptr->_pointer)->GetMemoryDC (ptr);
		return hr;
	}

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public readonly float GetPixelsPerDip () {
		var ptr = GetCurrentPointer ();
		var hr = ((FunctionPointer*) ptr->_pointer)->GetPixelsPerDip (ptr);
		return hr;
	}

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public readonly int SetPixelsPerDip (float pixelsPerDip) {
		var ptr = GetCurrentPointer ();
		var hr = ((FunctionPointer*) ptr->_pointer)->SetPixelsPerDip (ptr, pixelsPerDip);
		return hr;
	}

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public readonly int GetCurrentTransform (Matrix3x2F* transform) {
		var ptr = GetCurrentPointer ();
		var hr = ((FunctionPointer*) ptr->_pointer)->GetCurrentTransform (ptr, transform);
		return hr;
	}

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public readonly int GetCurrentTransform (out Matrix3x2F transform) {
		fixed (Matrix3x2F* _transform = &transform) {
			var ptr = GetCurrentPointer ();
			var hr = ((FunctionPointer*) ptr->_pointer)->GetCurrentTransform (ptr, _transform);
			return hr;
		}
	}

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public readonly int SetCurrentTransform (Matrix3x2F* transform) {
		var ptr = GetCurrentPointer ();
		var hr = ((FunctionPointer*) ptr->_pointer)->SetCurrentTransform (ptr, transform);
		return hr;
	}

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public readonly int GetSize (Vector2I* size) {
		var ptr = GetCurrentPointer ();
		var hr = ((FunctionPointer*) ptr->_pointer)->GetSize (ptr, size);
		return hr;
	}

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public readonly int GetSize (out Vector2I size) {
		fixed (Vector2I* _size = &size) {
			var ptr = GetCurrentPointer ();
			var hr = ((FunctionPointer*) ptr->_pointer)->GetSize (ptr, _size);
			return hr;
		}
	}

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public readonly int Resize (uint width, uint height) {
		var ptr = GetCurrentPointer ();
		var hr = ((FunctionPointer*) ptr->_pointer)->Resize (ptr, width, height);
		return hr;
	}

	[StructLayout (LayoutKind.Sequential)]
	readonly struct FunctionPointer {
		/// <summary>
		/// OFFSET 0
		/// </summary>
		public readonly delegate* unmanaged[Stdcall]<void*, Guid, void**, int> QueryInterface;
		/// <summary>
		/// OFFSET 1
		/// </summary>
		public readonly delegate* unmanaged[Stdcall]<void*, uint> AddRef;
		/// <summary>
		/// OFFSET 2
		/// </summary>
		public readonly delegate* unmanaged[Stdcall]<void*, uint> Release;
		/// <summary>
		/// OFFSET 3
		/// </summary>
		public readonly delegate* unmanaged[Stdcall]<void*, float, float, MeasuringMode, GlyphRun*, void*, uint, Vector4I*, int> DrawGlyphRun;
		/// <summary>
		/// OFFSET 4
		/// </summary>
		public readonly delegate* unmanaged[Stdcall]<void*, IntPtr> GetMemoryDC;
		/// <summary>
		/// OFFSET 5
		/// </summary>
		public readonly delegate* unmanaged[Stdcall]<void*, float> GetPixelsPerDip;
		/// <summary>
		/// OFFSET 6
		/// </summary>
		public readonly delegate* unmanaged[Stdcall]<void*, float, int> SetPixelsPerDip;
		/// <summary>
		/// OFFSET 7
		/// </summary>
		public readonly delegate* unmanaged[Stdcall]<void*, Matrix3x2F*, int> GetCurrentTransform;
		/// <summary>
		/// OFFSET 8
		/// </summary>
		public readonly delegate* unmanaged[Stdcall]<void*, Matrix3x2F*, int> SetCurrentTransform;
		/// <summary>
		/// OFFSET 9
		/// </summary>
		public readonly delegate* unmanaged[Stdcall]<void*, Vector2I*, int> GetSize;
		/// <summary>
		/// OFFSET 10
		/// </summary>
		public readonly delegate* unmanaged[Stdcall]<void*, uint, uint, int> Resize;
	}

	/// <inheritdoc />
	[MethodImpl (MethodImplOptions.AggressiveInlining)]
	public void Dispose () => Release ();
}
